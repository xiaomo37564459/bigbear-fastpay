-- =====================================================
-- Fast 易支付 数据库迁移脚本 V1.2（PostgreSQL）—— 密码存法从 MD5 换成 bcrypt
...
--   psql -h <host> -p 5432 -U <user> -d fastpay -f V1_2__password_hash_bcrypt_pg.sql
-- =====================================================

-- 放宽管理员密码字段，容纳 bcrypt（及未来更长的算法）
ALTER TABLE fp_admin ALTER COLUMN password TYPE VARCHAR(255);
COMMENT ON COLUMN fp_admin.password IS '密码（bcrypt 加密存储；兼容历史 MD5，登录成功后自动升级为 bcrypt）';

-- 放宽商户密码字段，和管理员保持同一套存法
ALTER TABLE fp_merchant ALTER COLUMN password TYPE VARCHAR(255);
