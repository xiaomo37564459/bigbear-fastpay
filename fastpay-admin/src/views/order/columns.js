...
export const ORDER_COLUMN_WIDTHS = {
  ...
  payType: 80,
  status: 76,
  // 注意：80 只保证**表头**和标签不折行。回调状态列还有一行副文案「已通知 N 次」，
  // 实测要 92 才放得下「已通知 999 次」，也就是说通知次数上两位数时这一行会折成两行。
  // 这是 MTM-165 之前就有的老问题，不是 MTM-168 引入的。这一轮没修：补到 92 要再从
  // 商户列挪 12px，而商户名正是 MTM-168 要救的那一列，两边不能同时满足，需要单独定夺。
  notify: 80,
...
export const MEASURED_MIN_WIDTHS = {
  ...
  fourCharHeader: 80,
  /** 副行「已通知 999 次」不折行；当前 notify 只给了 80，这条是已知缺口 */
  notifyCountLine: 92,
