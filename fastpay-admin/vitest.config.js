...
export default defineConfig({
  ...
  test: {
    ...
    //
    // 预打包的产物会写进 node_modules/.vite/vitest/deps/。默认情况下 7 个测试
    ...
    // 这个坑商户平台真踩过（MTM-251，PR #82 连红两次，质检本地冷缓存 3 次也红了 2 次），
    // 管理后台的配置和它出事前完全相同，只是测试文件少、撞上的概率低而已（MTM-264）。
    //
    // 改成一个一个文件跑，全程只有一个进程去建这份缓存，也就不存在互相抢。
    // 代价是不再并行，但预打包已经把大头省下来了，所以整体并不吃亏。
