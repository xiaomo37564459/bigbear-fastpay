...
## 十、代码合进主干之后：记得当场建一条「上线」任务

这一节是团队约定，不是选做。每次有代码合进 `master`，负责合并的人必须**在合并的当场**再开一条「上线」任务，把这次改动排上真正的发版。**别指望有人提醒你，也别等下次想起来。合并和建上线任务，是同一件事的两半。**

### 记死这四条

1. **验收通过 ≠ 用户能看到。** 质检把一条 issue 判「通过」，意思是代码没毛病、测试也过了。此时线上跑的还是旧版本 —— 这是我们说好的规矩，不是有人漏了什么
2. **谁把代码合进 `master`，谁就在合并的当场建一条「上线」任务。** 不是等别人提醒、不是等下次想起来。合并动作没做完，就还没走完
3. **那条上线任务必须写清楚三件事**：
   - 发的是哪一版（贴合并提交号，比如 `a7a88b2`）
   - 这次上线会顺带带上哪些别的改动（在 `master` 上、比上一次发版新出来的东西，全列出来）
   - 指派给 `梁运｜运维`
4. **原来那条 issue 该标完成就标完成** —— 它的活确实干完了。上线是新的一条任务，两者互不卡对方。别为了「等上线」就把 issue 一直挂着不关

### 为什么要写这条规矩

MTM-180 那次踩过一次坑：代码 8 月 19 号就进了主干，因为没人当场建上线任务，一直躺到 8 月 20 号早上才有人想起来发版。中间需求方 heory 亲自去线上点，看到的还是旧版本，白跑一趟。事情本身没错 —— 验收标准就是只管到「代码没问题」不管上线 —— 但「上线任务由谁在什么时候建」这一步之前一直靠人记得，这次就靠不住。所以现在写死：**合并的人负责建，建了才算合并完。**
