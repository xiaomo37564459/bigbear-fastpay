...
> 而后端返回的已经是 ISO-8601 文字日期 —— 照页面写代码的对接方会解析失败（MTM-258 修的就是它）。
> **教训：只发一半时，文档页面这类「跟着前端走的东西」会和后端行为对不上，别只盯着接口本身。**
>
...
- **跨版本升级**（比如线上还停在 `v1.2.0`，这次直接发 `v1.4.0`）：把中间每一版「新增」那一列的脚本**按版本从小到大依次跑一遍**。所有脚本重复跑都不会出问题（用了 `IF NOT EXISTS`），已经跑过的再跑一次不会中断部署。
  - ⚠️ **「重复跑都不会出问题」这条只对 PostgreSQL 成立。** `ADD COLUMN IF NOT EXISTS` 是 MariaDB 的写法，**原厂 MySQL 不支持**，照跑会报语法错 `ERROR 1064`。六个 `_mysql` 脚本里只有 `V1_1` 和 `V1_3` 用到它 —— **在原厂 MySQL 上执行这两个时，把里面的 `IF NOT EXISTS` 去掉再跑**，重复执行会报「列已存在」，属正常。（`V1_4` 和 `V1_6` 用的是 `CREATE TABLE IF NOT EXISTS`，原厂 MySQL 支持；`V1_2` / `V1_5` 是纯 `MODIFY COLUMN`：这四个都不受影响。）细节见下面 `V1_3` 那一段的最后一条。生产用的是 PostgreSQL，走不到这里。
  - `V1_2` / `V1_3` / `V1_5` / `V1_6` 只改表结构（放宽字段宽度、加列、加表），**不动任何已有数据**；`V1_1` 只多一句把新加的 `token_version` 列的空值补成 0（`UPDATE ... WHERE token_version IS NULL`，重复跑也没事）。
...
> ✅ **这张表要是没跟上，用命令现查，别猜。**
> 线上现在跑的是 `<旧版本>`、这次要发 `<新版本>`，那么下面这条命令列出来的，就是本次要留意的全部脚本：
>
> ```bash
> git diff --name-status <旧版本>..<新版本> -- fastpay-server/src/main/resources/db/migration/
> ```
>
> **没有任何输出 = 这次不用跑迁移。**
>
> 有输出的话，看每行开头那个字母：`A` 是**新增**的脚本，一定要跑；`M` 是**被改过**的老脚本，
> 得点开看看改了什么再决定（只改注释就不用管，改了 SQL 语句就要按情况补跑）。
>
> ⚠️ **别在这条命令上加 `--diff-filter=A`。** 加了就只看得见新增的脚本，
> **被改过的老脚本会被静悄悄漏掉**，而命令还是「正常」返回，看的人不会察觉。
> 这不是假设：`v1.7.0..v1.8.0` 之间 `V1_1__admin_account_management_mysql.sql` 就被改过一次
> （MTM-188 补了段注释），带 `--diff-filter=A` 查是空的、不带才看得见。
> 那次结论侥幸没错 —— 改的只是注释，而且是 MySQL 版本、生产根本用不上 ——
> 但换成改了 SQL 语句的情况，同一条命令就会让人放心地漏跑。（来源：MTM-258）
>
> 📌 **打完 tag 之后，顺手回来给这张表补一行，并且按版本号从小到大插到正确的位置。**
> 这张表是出事时照着做回退的依据，顺序错了容易看岔行。
> 这张表跟着 tag 一起走，别再出现「文档写着要跑、实际根本不用跑」这种对不上的情况。
...
**「登录失败次数限制」版本需要执行的迁移脚本清单**（`V1_6`）：

| 数据库 | 脚本 | 做了什么 |
| --- | --- | --- |
| PostgreSQL（生产） | `db/migration/V1_6__login_attempt_pg.sql` | 新增 `fp_login_attempt` 表（含唯一索引 + 二级索引），供 `LoginLimitService` 持久化「同一账号 / 同一 IP 连续失败几次、锁到什么时候」的状态；`ip:` 前缀的行是 IP 维度，无前缀的是账号维度 |
| MySQL（本地/老装机） | `db/migration/V1_6__login_attempt_mysql.sql` | 同上，MySQL 方言 |

步骤和前面几个完全一样：把 `scp` 和 `psql` 命令里的文件名换成 `V1_6__login_attempt_pg.sql`，其余照抄。

下面三条都是 `V1_6` 的注意事项：

> 💡 **只新增一张表，一行现有数据都不动**（原理和 `V1_3` 一致）。加表本身在 PG 上是元数据操作，不锁任何业务表，几毫秒完成。

> ⚠️ **顺序不能反**：必须先跑脚本、再换 jar。反过来做，新版后端一登录就会去读 `fp_login_attempt`，表不存在直接报错，用户既登不进也拿不到有意义的提示。

> 💡 **漏跑 `V1_6` 会怎样？** 后端启动本身不报（懒加载），但**任何人一登录就会 500**（连管理员和商户自己都进不去），所以别省。
...
<a id="加新迁移脚本必做两件事"></a>
> 🔴 **加了新的迁移脚本，必须同时做这两件事，缺一件等于白加：**
>
> 1. 在**上面那张「哪个版本要跑迁移」的表**里补一行，让发版的人知道这一版要跑它
> 2. 在 `deploy/verify-release.sh` 里那张**迁移检查表**（`MIGRATION_CHECKS=`）里补一行，让[发版验证脚本](RELEASE_VERIFY.md)查得出这一版有没有漏跑
>
> 第 2 件只需要加一行，格式和判定规则在那张表上面的注释里写着，代码一个字都不用改。
>
> **为什么单独强调**：这个脚本最早是手写的两段 `if`，只认 `V1_1` 和 `V1_2`。后来加了 `V1_3` / `V1_4` / `V1_5`，没人回来补，于是这三版漏跑了脚本照样报「全部正常」—— 尤其 `V1_4` 漏跑，等于那个「两人同价撞单认错人」的赔钱 bug 修复根本没生效，而跑完验证的人还以为一切妥当。改成表驱动之后，`deploy/verify-release.test.sh` 里有一条测试盯着这张表，少了哪一版会直接红（MTM-214）。
...
> 当前有效的那份是 `/root/fastpay-ops/ROLLBACK-v1.8.0.md`（2026-08-25 14:53 发 v1.8.0 前端那次，MTM-258）。
> `ROLLBACK-v1.8.0-backend-only.md`、`ROLLBACK-v1.7.0.md`、`ROLLBACK-v1.7.0-frontend-only.md`、
> `ROLLBACK-v1.6.0.md`、`ROLLBACK-v1.5.1.md` 都已在文件开头标成「已过期」，
> 并且**全部指向同一份当前有效文档** —— 别让人顺着旧指针一层层跳到另一份过期的上面去。
>
> ⚠️ 这一份的回退说明里专门写了一条**别人容易忽略的代价**：退回 `v1.7.0` 前端，
> 开发文档页面会重新写成错的「过期时间是一串数字」，而后端返回的是文字日期，
> **外部对接方照着写会解析失败**（MTM-176 / MTM-258 修的就是它）。
> 真要退，看一眼那份文档里「退回去会把哪个问题带回来」那一节。
> 连后端一起退的代价（会把「日志明文打印商户密钥」那个洞重新打开，MTM-239）写在
> `ROLLBACK-v1.8.0-backend-only.md` 里 —— 那份虽已标过期，后端回退步骤仍然可用。
