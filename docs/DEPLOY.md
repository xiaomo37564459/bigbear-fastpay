...
| `v1.5.0` | **无** | CI 流程和团队文档，不动表结构 |
| `v1.5.1` | `V1_4__pay_amount_uniqueness_pg.sql` | 新增 `fp_pending_pay_amount`、`fp_unmatched_notify` 两张表；**并且会改动已有订单数据**（回填 `pay_amount`、把重复的未支付订单强制关掉），跑之前先看下面那条 ⚠️ |
| `v1.6.0` | `V1_3__order_notify_result_pg.sql` | `fp_pay_order` 新增 `notify_result VARCHAR(1000)`、`notify_error VARCHAR(500)` 两列，把商户回调返回的内容和失败原因存进库 |

> 📌 **编号只是唯一的文件名，不代表发布先后。** 看上面两行就明白：`V1_4` 随 `v1.5.1` 发布，`V1_3` 随 `v1.6.0` 发布，**编号大的反而先上线**。
> 之所以会这样，是因为两个任务并行开发时各自取号，合到主干才发现撞号，事后只能顺延编号、不能改已经发出去的 tag。
> **要跑哪个脚本，永远以这张表为准，不要按文件名排序去猜。** 好在四个脚本互相独立、都能重复执行，跑的先后不影响结果。
> （防止再撞号的规矩见 MTM-197。）
...
- **跨版本升级**（比如线上还停在 `v1.2.0`，这次直接发 `v1.6.0`）：把中间每一版「新增」那一列的脚本**按发布版本从小到大依次跑一遍**（注意是按**版本号**排，不是按脚本文件名排 —— 原因见上面那条 📌）。所有脚本重复跑都不会出问题（用了 `IF NOT EXISTS`），已经跑过的再跑一次不会中断部署。
  - `V1_1`：加列 + 一句把 `token_version` 空值补成 0，重复跑没事
  - `V1_2`：只放宽字段宽度，不动已有数据
  - `V1_3`：只加两个可空列，不动已有数据
  - **`V1_4`：会改动已有订单数据** —— 回填 `pay_amount`，并把重复的未支付订单强制关掉。**跑之前先按脚本头部那两条 SQL 统计受影响行数。**
...
**库落后好几版要补跑时，按 `V1_1` → `V1_2` → `V1_3` → `V1_4` 的顺序跑一遍就行。** 四个脚本都是幂等的，已经跑过的再跑一遍不会出事；它们互相独立，先后顺序也不影响最终结果。

⚠️ **只有 `V1_4`（同价撞单那份）会改动已有订单数据** —— 它会回填 `pay_amount`，并把「同商户 + 同支付方式 + 同金额」的重复未支付订单强制关掉（`status` 0 → 3）。补跑到这一步之前，**先照脚本头部那两条 SQL 统计一下会影响多少行**，心里有数再执行。其余三个脚本只加表、加列、放宽字段宽度，不动任何已有数据。
