...
- **跨版本升级**（比如线上还停在 `v1.2.0`，这次直接发 `v1.4.0`）：把中间每一版「新增」那一列的脚本**按版本从小到大依次跑一遍**。所有脚本重复跑都不会出问题（用了 `IF NOT EXISTS`），已经跑过的再跑一次不会中断部署。
  - ⚠️ **「重复跑都不会出问题」这条只对 PostgreSQL 成立。** `ADD COLUMN IF NOT EXISTS` 是 MariaDB 的写法，**原厂 MySQL 不支持**，照跑会报语法错 `ERROR 1064`。五个 `_mysql` 脚本里只有 `V1_1` 和 `V1_3` 用到它 —— **在原厂 MySQL 上执行这两个时，把里面的 `IF NOT EXISTS` 去掉再跑**，重复执行会报「列已存在」，属正常。（`V1_4` 用的是 `CREATE TABLE IF NOT EXISTS`，原厂 MySQL 支持；`V1_2` / `V1_5` 是纯 `MODIFY COLUMN`：这三个不受影响。）细节见下面 `V1_3` 那一段的最后一条。生产用的是 PostgreSQL，走不到这里。
