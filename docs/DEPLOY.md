...
> ✅ **跑漏了不用靠记性 —— 发版验证脚本会替你逐个查。** `deploy/verify-release.sh` 会去库里把
> `V1_1`～`V1_5` 每一个该建的列 / 表挨个确认一遍，漏了哪个就点名说是哪个、缺什么、补跑哪个文件，
> 并且**以退出码 1 结束**（只有 `V1_2` 是提醒，因为它漏跑当天不会出故障）。查的是 PostgreSQL 还是
> MySQL，它按 `DB_URL` 的前缀自己判断。详见 [发完版怎么确认真的发好了](RELEASE_VERIFY.md)。
>
> ⚠️ **新加了迁移脚本，记得同时给验证脚本补一项检查**，否则漏跑它照样报绿 —— `V1_3`／`V1_4` 就漏查过一阵子（MTM-215）。
